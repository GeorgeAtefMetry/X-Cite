...
const Home = () => {
  ...
  useEffect(
    () =>
      onSnapshot(
        collection(db, "products/r0IPHZLeSwWjyANaykLp/digital-cards"),
        (snapshot) => {
          setDigitalCards(
  ...
  useEffect(
    () =>
      onSnapshot(
        collection(db, "products/phones-personal-audio/phones-personal-audio"),
        (snapshot) => {
  ...
  useEffect(
    () =>
      onSnapshot(
        collection(db, "products/televisions/televisions"),
        (snapshot) => {
          setTelevisions(
            snapshot.docs.map((doc) => ({ ...doc.data(), id: doc.id }))
          );
    ...
    <>
            ...
            <div className="col-lg-3 col-md-6" key={pro.id}>
              <div className="card">
                ...
                <div className="card-body">
                  <span className="card-text first float-left col-lg-10">
                    {pro.Name}
                  </span>
                  <span className="card-text second float-right col-lg-2">
                    Save Up to
                    <span className="float-right disc">{pro.discount}</span>
                  </span>
                </div>
              </div>
            </div>
              ...
              {digitalCards.map((pro) => (
                <SwiperSlide>
                  <div className="digital-card float-left" key={pro.id}>
                    <div>
                      <span className="best-seller float-left">
                        Best Seller
                      </span>
                    ...
                    <div class="card-body">
                      <span class="card-text name float-left col-lg-10">
                        {pro.name}
                      </span>
                      <span class="card-text price float-left col-lg-10">
                        {pro.price}
                      </span>
                      <div className="float-left col-lg-12">
                        <span class="card-text oldprice float-left col-lg-4">
                          {pro.oldPrice}
                        </span>
      ...
      <div className="phones-personal-audio">
        <Swiper
          ...
          // onSwiper={(swiper) => console.log(swiper)}
        >
          <div className="float-left phones-personal-heading">
          ...
          <div class="carousel-item active">
            <div className="row">
              {phonesAndPersonalAudio.map((pro) => (
                <SwiperSlide>
                  <div
                    className="phones-personal-audio float-left"
                    key={pro.id}
                  >
                    <div>
                      <span className="best-seller float-left">
